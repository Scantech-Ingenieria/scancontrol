...
static public function listarAlimentacionMdl($tabla) {
		...
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla a INNER JOIN sprockets s on s.id_sprockets=a.tipo_sprockets  INNER JOIN bandas b on b.id_banda=a.banda_modular_tipo");
		$sql -> execute();
		return $sql -> fetchAll();
	...
	static public function mdlCrearAlimentacion($tabla, $datos) {
		...
		$sql = Conexion::conectar()->prepare("INSERT INTO $tabla() VALUES (NULL,:tipo,:cantidadsprockets,:tipobandas,:bandasmedidas,:eje,:motorusillo,:motorcapacidad,:motorrpm)");
	...
	static public function mdlActualizarAlimentacion($tabla, $datos) {
			...
			$sql = Conexion::conectar()->prepare("UPDATE $tabla SET tipo_sprockets = :tipo,cantidad_sprockets = :cantidadsprockets,banda_modular_tipo = :tipobandas,banda_medidas = :bandasmedidas,eje = :eje,motor_usillo = :motorusillo,motor_capacidad = :motorcapacidad,motor_rpm = :motorrpm WHERE id_unidad_alim = :id");
