...
static public function listarCalidadMdl($tabla) {
	...
	static public function listarCalidadSprocketsMdl($tabla) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla a INNER JOIN sprockets s on s.id_sprockets=a.tipo_sprockets");
		$sql -> execute();
		return $sql -> fetchAll();
	}
	static public function listarCalidadBandasMdl($tabla) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla a INNER JOIN bandas b on b.id_banda=a.tipo_banda ");
		$sql -> execute();
		return $sql -> fetchAll();
	}
	static public function listarCalidadRodamientosMdl($tabla) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla a  INNER JOIN rodamientos r on r.id_rodamientos=a.tipo_descanso");
		$sql -> execute();
		return $sql -> fetchAll();
	}
	static public function listarCalidadSensorMdl($tabla) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla a INNER JOIN sensor se on se.id_sensor=a.tipo_sensores ");
		$sql -> execute();
		return $sql -> fetchAll();
	}
		static public function listarCalidadCilindrosMdl($tabla) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla a  INNER JOIN cilindros c on c.id_cilindros = a.cilindros ");
		$sql -> execute();
		return $sql -> fetchAll();
	}
		static public function listarCalidadMotorMdl($tabla) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla a INNER JOIN motor m on m.id_motor=a.tipo_motor");
		$sql -> execute();
		return $sql -> fetchAll();
	}

	static public function listarCalidadRegistroMdl($tabla) {
		$sql = Conexion::conectar()->prepare("SELECT * FROM $tabla a INNER JOIN sprockets s on s.id_sprockets=a.tipo_sprockets  INNER JOIN bandas b on b.id_banda=a.tipo_banda INNER JOIN rodamientos r on r.id_rodamientos=a.tipo_descanso INNER JOIN sensor se on se.id_sensor=a.tipo_sensores WHERE id_unidad IS NULL");
		$sql -> execute();
		return $sql -> fetchAll();
	}
	static public function mdlCrearCalidad($tabla, $datos) {
		$sql = Conexion::conectar()->prepare("INSERT INTO $tabla() VALUES (NULL, :puestos, :tiposprockets, :cantidadsprockets, :tipobandas, :medidabanda, :eje, :cilindros, :tipobotoneras, :cantidadbotoneras, :tiposensores, :cantidadsensores, :racors, :tipomotor, :tipodescanso,NULL)");
